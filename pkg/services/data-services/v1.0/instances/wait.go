...
func (ProvisionResponse) WaitForProvision(ctx context.Context, c *ClientWithResponses, r ProvisionResponse, projectID, instanceID string) *wait.Handler {
...
func (UpdateResponse) WaitForUpdate(ctx context.Context, c *ClientWithResponses, projectID, instanceID string) *wait.Handler {
	return wait.New(func() (res interface{}, done bool, err error) {
		s, err := c.Get(ctx, projectID, instanceID)
		if err = validate.Response(s, err, "JSON200"); err != nil {
			if strings.Contains(err.Error(), client_timeout_err) ||
...
func (DeprovisionResponse) WaitForDeprovision(ctx context.Context, c *ClientWithResponses, projectID, instanceID string) *wait.Handler {
	return wait.New(func() (res interface{}, done bool, err error) {
		s, err := c.Get(ctx, projectID, instanceID)
		if err = validate.Response(s, err, "JSON200"); err != nil {
			if strings.Contains(err.Error(), client_timeout_err) ||
